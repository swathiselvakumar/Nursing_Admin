...
export default function PrelimsInstitution() {
  ...
  const Send = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost/Nursing/controllers/api/admin/get/A_ViewPrelimsPaper.php`,
        {
          adminId:email,
          id: sno,
        }
      ...
      for (let j = i; j < Math.min(i + 3, mc.length); j++) {
        ...
        rowItems.push(
          <Col xs={12} sm={12} md={6} lg={4} xl={4} key={mcq.sno} id={id} className="MainBox">
            <div className="box">
              <NavLink to={`/prelimstablepage/${mcq.sno}/${sno}`} style={{ textDecoration: "none" }}>
                <button
                  style={{
            ...
            <NavLink 
              to={`/prelimsquestion/${mcq.sno}/${sno}`}
  ...
  const CardDelete = async (sno) => {
    ...
    try {
      const res = await axios.delete(
        `http://localhost/Nursing/controllers/api/admin/delete/A_deletePrelimsPaper.php`,
        {
          data: {
            adminId:email,
            paperId: sno,
  ...
  return (
    <div style={{ backgroundColor: "white", padding: "10px" }}>
      ...
      <div className="BtnBox">
        <NavLink to={`/addprelims/${sno}/${lastId}`}>
